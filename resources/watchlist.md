...
- 標準：`- [name](link) — 一句 tagline。為什麼想看：reason。Added: YYYY-MM-DD`
- 高信號資源加 ⭐ 前綴（10k+ stars / 大廠 / 跨工具標準）
- 採用前需 verify 的 blocker 用 `Note: <一句>` 子句（只標記、不評估；完整評估在 promote 時做）

一行為原則。不要在這層做完整判斷 —— 那是 promote 後 `resources/repos/` 的事。
